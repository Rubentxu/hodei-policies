...
pub enum AuthServiceError {
    ...
    PolicyAdd(#[from] cedar_policy::PolicySetError),
    #[error("Policy ID not found for removal")]
    PolicyIdNotFound,
}
...
pub trait CacheInvalidation: Send + Sync {
    async fn publish_invalidation(&self) -> Result<(), AuthServiceError>;
    async fn subscribe_to_invalidation(&self, on_message: Box<dyn Fn() + Send + Sync + 'static>) -> Result<(), AuthServiceError>;
}
...
impl PolicyAdapter for PostgresAdapter {
    async fn load_policies(&self) -> Result<PolicySet, AuthServiceError> {
        ...
        for record in records {
            ...
            let p = Policy::parse(Some(policy_id), &content).map_err(|e| {
                AuthServiceError::Io(std::io::Error::new(
        ...
        let policy_set = PolicySet::from_policies(policies).map_err(AuthServiceError::PolicyAdd)?;
    ...
    async fn create_policy(&self, content: String) -> Result<String, AuthServiceError> {
        ...
        sqlx::query("INSERT INTO policies (id, content) VALUES ($1, $2)")
            .bind(&policy_id)
            .bind(&content)
            .execute(&self.db)
            .await?;
    ...
    async fn update_policy(&self, id: String, content: String) -> Result<(), AuthServiceError> {
        let result = sqlx::query("UPDATE policies SET content = $1 WHERE id = $2")
            .bind(&content)
            .bind(&id)
            .execute(&self.db)
            .await?;
...
pub struct AuthorizationService {
    ...
    cache_invalidator: Arc<dyn CacheInvalidation>,
}
...
impl AuthorizationService {
    pub async fn new(
        ...
        let schema =
            Arc::new(Schema::from_json_str(&schema_str).map_err(|e| AuthServiceError::Schema(e))?);
        ...
        service.start_invalidation_listener();
        Ok(service)
    }
    ...
    fn start_invalidation_listener(&self) {
        ...
        tokio::spawn(async move {
            let _ = cache_invalidator
                .subscribe_to_invalidation(Box::new(move || {
                    let policies = policies.clone();
                    let adapter = adapter.clone();
                    tokio::spawn(async move {
                        if let Ok(new_policies) = adapter.load_policies().await {
                            let mut policies = policies.write().await;
                            *policies = new_policies;
                            tracing::info!("✅ Policies reloaded from cache invalidation");
                        }
                    });
                }))
                .await;
    ...
    pub async fn create_policy(&self, content: String) -> Result<String, AuthServiceError> {
        ...
        // Publish invalidation message after DB update
        self.cache_invalidator.publish_invalidation().await?;
        // The local cache will be reloaded by the listener
        Ok(policy_id)
    }
